...
read_f24_single_file_ <- function(file_path) {
  ...
  # game infos
  game <- getNodeSet(doc, "//Game")
  game_attributs <- 
    sapply(game, FUN = function(g) {
      comp <- xmlGetAttr(g, "competition_name")
      season <- xmlGetAttr(g, "season_name")
      j <- xmlGetAttr(g, "matchday")
      away <- xmlGetAttr(g, "away_team_name")
      home <- xmlGetAttr(g, "home_team_name")
  ...
  for (e in 1:nrow(events_tibble)) {
    qualifiers_events_e <- getNodeSet(events[[e]], "./Q")
    
    qualifier_tmp <- NULL
    if (length(qualifiers_events_e) > 0) {
      for (q in 1:length(qualifiers_events_e)) {
        if (is.null(qualifier_tmp)) {
          qualifier_tmp <- xmlToList(qualifiers_events_e[[q]]) %>% 
            as_tibble_row()  
        } else {
          qualifier_tmp <- xmlToList(qualifiers_events_e[[q]]) %>% 
            as_tibble_row() %>%
            bind_rows(qualifier_tmp) 
        }
      }
      
      qualifier_tmp <- qualifier_tmp %>%
        mutate(parent_event_id = xmlAttrs(events[[e]])[['id']])
...
read_f24 <- function(file_path=NULL, dir_path=NULL) {
  ...
  if (!is.null(dir_path)){ # case of a dir is provided
    ...
    events_tibble_cplt <- read_f24_single_file_(file_path)
  }
